...
const sidebar = ({ expand, setExpand }) => {
    ...
    return (
        <>
            {/* ✅ FIXED: Overlay backdrop to prevent white space when scrolling on mobile */}
            {expand && (
                <div 
                    className="md:hidden fixed inset-0 bg-[#212327] z-40"
                    onClick={() => setExpand(false)}
                />
            )}

            <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:fixed max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
                <div>
                    <div className={`flex ${expand ? 'flex-row gap-10' : 'flex-col items-center gap-8'}`}>
                        <Image className={expand ? 'w-36' : 'w-10'} src={expand ? assets.logo_text : assets.logo_icon} alt='' />

                        <div onClick={() => expand ? setExpand(false) : setExpand(true)} className='group relative flex items-center justify-center hover:bg-gray-500/20 transition-all duration-300 h-9 w-9 rounded-lg'>
                    ...
                    <button onClick={createNewChat} className={`mt-8 flex items-center justify-center cursor-pointer ${expand ? 'bg-primary hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max' : 'group relative flex items-center justify-center hover:bg-gray-500/20 transition-all duration-300 h-9 w-9 rounded-lg'}`}>
                    ...
                    <div className={`mt-8 text-white/25 text-sm ${expand ? 'block' : 'hidden'} max-h-96 overflow-y-auto`}>
                ...
                <div>
                    <div className={`flex items-center cursor-pointer group relative ${expand ? 'gap-1 text-white/80 text-sm p-2.5 border border-primary rounded-lg hover:bg-white/10 cursor-pointer' : 'flex items-center justify-center hover:bg-gray-500/20 transition-all duration-300 h-9 w-9 rounded-lg'}`}>
                    ...
                    <div onClick={user ? null : openSignIn} className={`flex items-center ${expand ? 'hover:bg-white/10 rounded-lg' : 'justify-center w-full'} gap-3 text-white/60 text-sm p-2 mt-2 cursor-pointer ${expand ? '' : 'flex justify-center'}`}>
